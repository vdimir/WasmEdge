...
void setLoggingCallback(std::function<void(const spdlog::details::log_msg &)> Callback) {
    auto Callback_sink = std::make_shared<spdlog::sinks::callback_sink_mt>(Callback);
    spdlog::set_default_logger(std::make_shared<spdlog::logger>("WasmEdge", Callback_sink));
}
