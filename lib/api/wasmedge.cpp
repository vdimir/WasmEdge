...
WASMEDGE_CAPI_EXPORT void WasmEdge_LogSetCallback(WasmEdge_LogCallback_t Callback) {
    WasmEdge::Log::setLoggingCallback([Callback](const spdlog::details::log_msg &Msg) {
        WasmEdge_LogMessage Message;

        Message.Message = WasmEdge_String{/* Length */ static_cast<uint32_t>(Msg.payload.size()),
                                          /* Buf */ Msg.payload.data()};
        Message.LoggerName = WasmEdge_String{/* Length */ static_cast<uint32_t>(Msg.logger_name.size()),
